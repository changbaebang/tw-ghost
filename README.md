...
- **`artifactLocation.uri`** is POSIX and percent-encoded per segment (a space becomes `%20`, `#`
  ...
  path when no workspace covers the file. **Such a result is kept, and tw-ghost warns once per run
  ...
  not change the exit code. Run from the repository root, or set `GITHUB_WORKSPACE`, and the path
  becomes repo-relative and the warning goes away.
